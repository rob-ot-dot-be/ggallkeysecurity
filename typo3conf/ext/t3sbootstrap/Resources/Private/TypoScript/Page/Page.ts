...
temp.globalSettings {
	...
	navigation {
		...
		megaMenu.enable = {$plugin.t3sbootstrap_configuration.navigation.megaMenu.enable}
		megaMenu.image.width = {$plugin.t3sbootstrap_configuration.navigation.megaMenu.image.width}
		megaMenu.image.height = {$plugin.t3sbootstrap_configuration.navigation.megaMenu.image.height}
	}
...
temp.jsFooterInline {
	...
	1.if.isTrue = {$plugin.t3sbootstrap_configuration.styles.navbar.style.fixed.custom}
	5 = TEXT
...
page {
	...
	bodyTagCObject {
		...
		30 = TEXT
		30 {
			value = data-spy="scroll" data-target="#scrollspy"
			noTrimWrap = | ||
			if.isInList.data = TSFE:id
			if.value = {$plugin.t3sbootstrap_configuration.pages.affix.uidList}
		}
