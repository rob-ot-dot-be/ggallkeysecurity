...
class VideoController extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	public function render_video($conf) {
		...
		if (\TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('7.4')) {
			...
			$pageRenderer->addJsFooterFile($videoJsPath);
		} else {
		...
		if (\TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('7.4')) {
			$pageRenderer->addCssFile($videoCssPath, 'stylesheet','all','', TRUE);
		} else {
		...
		if ( $source == 1 ) {
			...
			$video .= '<video id="video_'.$uid.'" width="675" height="380" class="video-js vjs-default-skin embed-responsive-item" controls '.$previewImage.' data-setup '.$autoplay.' '.$preload.' '.$loop.'>';
		    ...
		    $video .= '<video id="video_'.$uid.'" src="" width="675" height="380" class="video-js vjs-default-skin embed-responsive-item '.$centerPlayButton.'" controls data-setup=\'{ "techOrder": ["youtube"], "src": "http://www.youtube.com/watch?v='.$youtube.'" }\' '.$autoplay.' '.$preload.' '.$loop.'>';
			...
			if (\TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('7.4')) {
				$pageRenderer->addJsFooterFile($youtubePath);
			} else {
			...
			$video .= '<video id="vid1_'.$uid.'" src="" width="675" height="380" class="video-js vjs-default-skin embed-responsive-item" controls data-setup=\'{ "techOrder": ["vimeo"], "src": "https://vimeo.com/'.$vimeo.'" }\' '.$autoplay.' '.$preload.' '.$loop.'>';
			...
			if (\TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('7.4')) {
				$pageRenderer->addJsFooterFile($vimeoPath);
			} else {
